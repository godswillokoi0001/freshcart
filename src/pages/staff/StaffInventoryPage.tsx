...
export function StaffInventoryPage() {
  ...
  const filtered = React.useMemo(() => {
    ...
    switch (sort) {
      ...
      case "status": list.sort((a, b) => a.stockStatus.localeCompare(b.stockStatus)); break
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-3">
          <div className="relative min-w-0 flex-1 sm:max-w-xs">
            ...
            <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search by name or SKU…" className="h-10 pl-9" />
          </div>
      ...
      <div className="rounded-lg border border-navy-200 bg-white overflow-hidden">
        <Table>
          ...
          <TableBody>
            {filtered.map((p) => (
              <TableRow
                key={p.id}
                className={cn("hover:bg-navy-50 cursor-pointer", selectedRow === p.id && "bg-fresh-50")}
                onClick={() => setSelectedRow((prev) => (prev === p.id ? null : p.id))}
              >
                <TableCell className="w-12">
                  <div className="h-10 w-10 rounded bg-navy-50 flex items-center justify-center">
                    <span className="text-lg">{p.categoryName === "Rice" ? "🍚" : p.categoryName === "Milk" ? "🥛" : "🛒"}</span>
                  </div>
                </TableCell>
                <TableCell>
                  <div>
                    <p className="font-medium text-navy-900">{p.name}</p>
                    <p className="text-xs text-navy-400 font-mono">{p.sku}</p>
                  </div>
                </TableCell>
        ...
        {filtered.length === 0 && (
          <div className="p-8 text-center text-navy-500">No products match your filters.</div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-navy-900">Product Details</h2>
              <Button variant="ghost" size="icon" onClick={() => setSelectedRow(null)}><XCircle className="h-5 w-5" /></Button>
            </div>
            <div className="mt-4 space-y-3">
              {(() => {
                const p = products.find((x) => x.id === selectedRow)!
                return (
                  <>
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-navy-500">{p.sku}</p>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div><span className="text-navy-500">Category:</span> <span className="font-medium">{p.categoryName}</span></div>
                      <div><span className="text-navy-500">Brand:</span> <span className="font-medium">{p.brand}</span></div>
                      <div><span className="text-navy-500">Unit:</span> <span className="font-medium">{p.unit}</span></div>
