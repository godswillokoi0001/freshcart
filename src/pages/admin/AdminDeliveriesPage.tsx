    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-col sm:flex-row flex-wrap gap-2.5 sm:items-center">
          <div className="relative w-full sm:w-64">
            ...
            <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search order # or customer…" className="h-10 pl-9 w-full" />
          </div>
      ...
      <div className="grid gap-6 lg:gap-8 lg:grid-cols-[2fr_1fr]">
        <div className="space-y-6">
          <section>
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-navy-900">Pending Assignments</h2>
            </div>
            <div className="rounded-lg border border-navy-200 bg-white divide-y divide-navy-100 shadow-2xs overflow-hidden">
              {filtered.filter(d => d.status === "UNASSIGNED").map(d => (
                <div key={d.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3.5 sm:p-4 hover:bg-navy-50/70 transition-colors">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-navy-50 text-navy-400">
                      <Truck className="h-5 w-5" />
                    </div>
                    <div className="min-w-0">
                      <p className="font-semibold text-navy-900 text-sm">{d.orderId}</p>
                      <p className="text-xs text-navy-500 truncate">{d.customerName} · {d.area} · {d.itemsCount} items</p>
                    </div>
                  </div>
                  <div className="flex items-center justify-between sm:justify-end gap-2 shrink-0">
                    <span className="text-sm font-bold text-navy-900">{formatNaira(d.deliveryFee)}</span>
                    <Badge variant="destructive">UNASSIGNED</Badge>
                  </div>
                </div>
              ))}
              {filtered.filter(d => d.status === "UNASSIGNED").length === 0 && (
                <div className="p-6 text-center text-sm text-navy-500">No pending assignments</div>
              )}
            </div>
          </section>
          ...
          <section>
            ...
            <div className="rounded-lg border border-navy-200 bg-white divide-y divide-navy-100 shadow-2xs overflow-hidden">
              {filtered.filter(d => d.status !== "UNASSIGNED" && d.status !== "DELIVERED").map(d => (
                <Link key={d.id} to="#" className="block p-3.5 sm:p-4 hover:bg-navy-50/70 transition-colors">
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="font-semibold text-navy-900 text-sm truncate">{d.orderId}</p>
                        <p className="text-xs text-navy-500 truncate">{d.customerName} · {d.area} · {d.itemsCount} items</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <DeliveryStatusBadge status={d.status} />
                      <ChevronRight className="h-4 w-4 text-navy-300 hidden sm:block" />
                    </div>
                  </div>
                </Link>
              ))}
              {filtered.filter(d => d.status !== "UNASSIGNED" && d.status !== "DELIVERED").length === 0 && (
                <div className="p-6 text-center text-sm text-navy-500">No active deliveries</div>
              )}
            </div>
          </section>
              ...
              {filtered.filter(d => d.status === "DELIVERED").slice(0, 10).map(d => (
                <div key={d.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3.5 sm:p-4 hover:bg-navy-50/70 transition-colors">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="font-semibold text-navy-900 text-sm">{d.orderId}</p>
                      <p className="text-xs text-navy-500 truncate">{d.customerName} · {d.area} · {formatNaira(d.deliveryFee)}</p>
                    </div>
                  </div>
                  <Badge variant="success" className="self-start sm:self-center">DELIVERED</Badge>
                </div>
              ))}
              {filtered.filter(d => d.status === "DELIVERED").length === 0 && (
                <div className="p-6 text-center text-sm text-navy-500">No deliveries completed yet</div>
              )}
            </div>
          </section>
        </div>
        ...
        <div className="space-y-6 lg:sticky lg:top-24 lg:self-start">
          <section className="rounded-lg border border-navy-200 bg-white p-4 sm:p-6 shadow-2xs">
            <h2 className="text-lg font-semibold text-navy-900">Available Riders</h2>
            <div className="mt-4 space-y-2">
              {availableRiders.map(r => (
                <div key={r.id} className="flex items-center justify-between gap-2 p-3 rounded-lg border border-navy-100 hover:bg-navy-50 transition-colors">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="font-semibold text-navy-900 text-sm truncate">{r.name}</p>
                      <p className="text-xs text-navy-500 truncate">{r.vehicle}</p>
                    </div>
                  </div>
          ...
          <section className="rounded-lg border border-navy-200 bg-white p-4 sm:p-6 shadow-2xs">
            <h2 className="text-lg font-semibold text-navy-900">Quick Assign</h2>
            <p className="mt-1 text-xs text-navy-500">Select a delivery and a rider, then click Assign.</p>
