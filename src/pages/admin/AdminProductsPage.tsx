      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-col sm:flex-row flex-wrap gap-2.5 sm:items-center">
          <div className="relative w-full sm:w-60">
            ...
            <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name or SKU…" className="h-10 pl-9 w-full" />
          </div>
          <div className="grid grid-cols-2 gap-2 sm:flex sm:gap-2.5">
            <Select value={statusFilter} onValueChange={(value) => setStatusFilter(value as typeof statusFilter)}>
              ...
              <SelectContent><SelectItem value="all">All</SelectItem><SelectItem value="ACTIVE">Active</SelectItem><SelectItem value="ARCHIVED">Archived</SelectItem></SelectContent>
            </Select>
          ...
          <Button onClick={handleCreate} className="w-full sm:w-auto"><Plus className="h-4 w-4" /> Add Product</Button>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[700px]">
            ...
            <tbody className="divide-y divide-navy-100 text-sm">
              {filtered.map(p => (
                <tr key={p.id} className="hover:bg-navy-50/70 transition-colors">
                  ...
                  <td className="p-3 text-sm text-navy-500">{p.categoryName}</td>
                  <td className="p-3 text-sm text-navy-500">{p.brand}</td>
                  ...
                  <td className="p-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <Button variant="ghost" size="icon" onClick={() => handleEdit(p)} title="Edit"><Edit className="h-4 w-4" /></Button>
                      <Button variant="ghost" size="icon" className="text-danger-600 hover:text-danger-700" title="Delete"><Trash2 className="h-4 w-4" /></Button>
                    </div>
                  </td>
                </tr>
              ))}
          ...
          {filtered.length === 0 && (
            <div className="p-8 text-center text-sm text-navy-500">No products found</div>
          )}
        </div>
      </div>
